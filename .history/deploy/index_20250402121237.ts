  ...
  run() {
    ...
    this.ls().then(async (fileList: FileInfo[]) => {
      ...
      ssh.putFile(`${this.localPath}\\${this.fileName}.zip`, `${this.remotePath}/${this.fileName}_${version}.zip`).then(async () => {
        ...
        console.log("更新版本...");
        // 删除dist文件夹
        await ssh.execCommand(`rm ${this.fileName} -rf`, { cwd: this.remotePath }).then(() => {
          // 解压最新版
          await ssh.execCommand(`unzip -o ${this.fileName}_${version}.zip -d ${this.fileName}`, { cwd: this.remotePath }).then(() => {
            // 更改名称 加_s 表示部署的版本
            ssh.execCommand(`mv ${this.fileName}_${version}.zip ${this.fileName}_${version}_s.zip`, { cwd: this.remotePath }).then(() => {
              console.log("版本更新完成！");
              // 断开连接
              ssh.dispose();
              process.exit(0);
    ...
    this.ls().then(async (fileList: FileInfo[]) => {
      ...
      ssh.execCommand(`rm ${this.fileName} -rf`, { cwd: this.remotePath }).then(() => {
        ...
        ssh.execCommand(`unzip -o ${this.fileName}_${version}.zip -d ${this.fileName}`, { cwd: this.remotePath }).then(() => {
          // 更改名称 加_s 表示部署的版本
          ssh.execCommand(`mv ${this.fileName}_${version}.zip ${this.fileName}_${version}_s.zip`, { cwd: this.remotePath }).then((res) => {
            console.log("版本更新完成！");
            // 断开连接
            ssh.dispose();
            process.exit(0);
